...
class CompositeAtom(Atom):
    ...
    def get1(self, k):
        for a in self.body:
            ...
            raise Exception('atom not found: '+str(k))
    def get(self, *keys):
        atom = self
        for k in keys:
            atom = atom.get1(k)
        return atom
    def get_all(self, k):
        return [x for x in self.body if x.type == k]
...
def read_mvhd(stream, size, left, type):
    ...
    left -= 80
    assert left == 0
    return VariableAtom(b'mvhd', size, body, [(b'duration', 16, duration)])
...
def read_tkhd(stream, size, left, type):
    ...
    left -= 60
    assert left == 0
    return VariableAtom(b'tkhd', size, body, [(b'duration', 20, duration)])
...
def read_mdhd(stream, size, left, type):
    ...
    assert left == 0
    return VariableAtom(b'mdhd', size, body, [(b'duration', 16, duration)])
...
def read_mp4a(stream, size, left, type):
    ...
    atom = read_atom(stream)
    assert atom.type == b'esds',atom.type
    left -= atom.size
...
def read_mp4(stream):
    ...
    moov = [x for x in atoms if x.type == b'moov']
    mdat = [x for x in atoms if x.type == b'mdat']
    assert len(moov) == 1
    assert len(mdat) == 1
    moov = moov[0]
    mdat = mdat[0]
...
def merge_stts(samples_list):
    ...
    counts, durations = list(zip(*sample_list))
...
def merge_stss(samples, sample_number_list):
    ...
    for samples, sample_number_list in zip(samples, sample_number_list):
        results.extend([start + x for x in samples])
...
def merge_moov(moovs, mdats):
    mvhd_duration = 0
    for x in moovs:
        mvhd_duration += x.get(b'mvhd').get(b'duration')
    tkhd_durations = [0, 0]
    mdhd_durations = [0, 0]
    for x in moovs:
        traks = x.get_all(b'trak')
        assert len(traks) == 2
        tkhd_durations[0] += traks[0].get(b'tkhd').get(b'duration')
        tkhd_durations[1] += traks[1].get(b'tkhd').get(b'duration')
        mdhd_durations[0] += traks[0].get(b'mdia', b'mdhd').get(b'duration')
        mdhd_durations[1] += traks[1].get(b'mdia', b'mdhd').get(b'duration')
    ...
    moov.get(b'mvhd').set(b'duration', mvhd_duration)
    trak0 = moov.get_all(b'trak')[0]
    trak1 = moov.get_all(b'trak')[1]
    trak0.get(b'tkhd').set(b'duration', tkhd_durations[0])
    trak1.get(b'tkhd').set(b'duration', tkhd_durations[1])
    trak0.get(b'mdia', b'mdhd').set(b'duration', mdhd_durations[0])
    trak1.get(b'mdia', b'mdhd').set(b'duration', mdhd_durations[1])
    ...
    stco0 = [x + new_mdat_start for x in stco0]
    stco1 = [x + new_mdat_start for x in stco1]
    stco_atom = trak0.get(b'mdia', b'minf', b'stbl', b'stco')
    stco_atom.body = stss_atom.body[0], stco0
    stco_atom = trak1.get(b'mdia', b'minf', b'stbl', b'stco')
    stco_atom.body = stss_atom.body[0], stco1
...
def merge_mp4s(files, output):
    ...
    moovs = [x[1] for x in mp4s]
    mdats = [x[2] for x in mp4s]
